...
const { black, blueBright, cyanBright, greenBright, grey, magenta, redBright, yellow } = require("chalk");
...
function format(d) {
  return (splice(d.getFullYear().toString(), 0, 2, "") +
    "-" +
    pad(d.getMonth() + 1, 2) +
    "-" +
    pad(d.getDate(), 2) +
...
class Logger {
  static log(content, options = {}) {
    ...
    if (!options.shard) {
      options.shard = "Manager";
    }

    if (typeof options.shard === "number" || !isNaN(parseInt(options.shard, 10))) {
